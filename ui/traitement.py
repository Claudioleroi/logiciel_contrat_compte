...
def insert_transaction(code_journal, auteur, destinataire, libelles, num_piece, montant, date, time):
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO transactions (code_journal, auteur, destinataire,  libelles, num_piece, montant, date, time)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (code_journal, auteur, destinataire, libelles, num_piece, montant, date, time))
    conn.commit()
    conn.close()
...
class TraitementTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Code Journal", "Auteur", "Destinataire", "Libellés", "Numéro de la pièce", "Montant", "Date", "Time"
        ])
    ...
    def save_data(self):
        ...
        if not (auteur and destinataire  and montant and date):
            QMessageBox.warning(self, 'Incomplete Data', 'Please fill in all required fields.')
            return

        try:
            insert_transaction(code_journal, auteur, destinataire,  libelles, num_piece, montant, date, time)
